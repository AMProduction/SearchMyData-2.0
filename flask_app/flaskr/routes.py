...
def home():
    ...
    return render_template(
        "home.html", form=form, status=True, index=True, now=datetime.utcnow()
    )
...
def show_collections_info():
    ...
    return render_template(
        "info.html", result=registers_info, isExpired=expiration, now=datetime.utcnow()
    )


@app.route("/result", methods=["POST", "GET"])
def show_search_results():
    from .src.data_action import (
    ...
    session_variables = [
        "result_count_missing_persons",
        "result_count_wanted_persons",
        "result_count_debtors",
        "result_count_lustrated",
        "pages_count",
    ]
    if not any(variable in session_variables for variable in session):
        # get search result count
        result_count_missing_persons = get_search_result_count(
            MissingPerson, search_string
        )
        ...
        result_count_wanted_persons = get_search_result_count(
            WantedPerson, search_string
        )
        ...
        pages_count = max(
            pages_for_lustrated,
            pages_for_debtors,
            pages_for_wanted_persons,
            pages_for_missing_persons,
        )
    ...
    return render_template(
        "result.html",
        now=datetime.utcnow(),
        result_MissingPersons=result_missing_persons,
        result_WantedPersons=result_wanted_persons,
        result_Debtors=result_debtors,
        ...
        result_count_debtors=result_count_debtors,
        result_count_lustrated=result_count_lustrated,
    )
